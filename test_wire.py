...
class OrthodonticWireGenerator:
    ...
    def detect_individual_teeth(self):
        ...
        temp_extent = extent.copy(); temp_extent[self.lr_axis] = -np.inf
        ...
        print("--- Initial Segment Detection (before sorting) ---") # DIAGNOSTIC HEADER 1
        for i in range(num_segments):
            ...
            if len(seg_verts)>30:
                ...
                tooth_info = {'center':t_center,'vertices':seg_verts,'label':len(self.teeth),'type':'unknown',
                ...
                # DIAGNOSTIC PRINT 1 (Item 1)
                print(f"  Raw segment: Angle {np.degrees(tooth_info['angle']):.1f}, AP {tooth_info['ap_position']:.1f}, LR_center {tooth_info['center'][self.lr_axis]:.1f}, NumVertices {len(tooth_info['vertices'])}, Label {tooth_info['label']}")

        self.teeth.sort(key=lambda t: t['angle'])
        # DIAGNOSTIC PRINT 2 (Item 2)
        print("\n--- Teeth after sorting by angle (before classification) ---")
        for i,t in enumerate(self.teeth):
            t['angular_order'] = i
            print(f"  Sorted Tooth {t['angular_order']}: Angle {np.degrees(t['angle']):.1f}, AP {t['ap_position']:.1f}, Original Label: {t['label']}")

        num_teeth = len(self.teeth)
        # DIAGNOSTIC PRINT (Item 3)
        print(f"\n--- Incisor Classification Logic ---")
        print(f"  Number of teeth for classification: {num_teeth}")
        if num_teeth > 0:
            sorted_ap_teeth = sorted(self.teeth, key=lambda t:t['ap_position'], reverse=True)
            print(f"  Sorted AP Teeth (angular_order, ap_pos, type): {[(t['angular_order'], round(t['ap_position'],1), t['type']) for t in sorted_ap_teeth]}")

            incisor_candidates = [sorted_ap_teeth[i] for i in range(min(num_teeth, 6))]
            print(f"  Incisor candidates by AP (angular_order, ap_pos, type): {[(t['angular_order'], round(t['ap_position'],1), t['type']) for t in incisor_candidates]}")

            min_ao,max_ao = int(num_teeth*0.2), int(num_teeth*0.8)
            print(f"  Min/Max angular index for centrality filter: {min_ao}/{max_ao}")

            potential_incisors = [t for t in incisor_candidates if min_ao <= t['angular_order'] <= max_ao]
            print(f"  Potential incisors after angular filter (angular_order, ap_pos, type): {[(t['angular_order'], round(t['ap_position'],1), t['type']) for t in potential_incisors]}")

            num_inc_cls = 0
            if len(potential_incisors) >= 4: num_inc_cls = min(len(potential_incisors), 6)
            elif len(incisor_candidates) >= 4: potential_incisors = incisor_candidates; num_inc_cls = min(len(potential_incisors), 6)
            else: potential_incisors = incisor_candidates; num_inc_cls = len(potential_incisors)
            print(f"  Number of incisors to classify: {num_inc_cls}")

            final_incisors_candidates = potential_incisors if potential_incisors or num_inc_cls > 0 else incisor_candidates # Ensure we have candidates if num_inc_cls > 0
            final_incisors = sorted(final_incisors_candidates[:num_inc_cls], key=lambda t:t['angular_order'])
            print(f"  Final incisors selected for typing (angular_order, ap_pos, type): {[(t['angular_order'], round(t['ap_position'],1), t['type']) for t in final_incisors]}")

            if final_incisors:
                median_ao_incisors = np.median([t['angular_order'] for t in final_incisors])
                # Provisionally assign all as lateral, then find centrals
                for t in final_incisors:
                    t['type']='lateral_incisor'
                    # print(f"    Incisor classification: Tooth {t['angular_order']} (AP: {t['ap_position']:.1f}) provisionally assigned type: {t['type']}")


                # Sort by distance to median angular order to find centrals
                final_incisors.sort(key=lambda t: abs(t['angular_order']-median_ao_incisors))
                if len(final_incisors)>=1:
                    final_incisors[0]['type']='central_incisor'
                    print(f"    Incisor classification: Tooth {final_incisors[0]['angular_order']} (AP: {final_incisors[0]['ap_position']:.1f}) assigned type: {final_incisors[0]['type']}")
                if len(final_incisors)>=2:
                    final_incisors[1]['type']='central_incisor'
                    print(f"    Incisor classification: Tooth {final_incisors[1]['angular_order']} (AP: {final_incisors[1]['ap_position']:.1f}) assigned type: {final_incisors[1]['type']}")

                # Re-sort by angular_order to print remaining laterals
                final_incisors.sort(key=lambda t: t['angular_order'])
                for t in final_incisors:
                    if t['type'] == 'lateral_incisor': # If it wasn't made a central
                         print(f"    Incisor classification: Tooth {t['angular_order']} (AP: {t['ap_position']:.1f}) assigned type: {t['type']}")

            # DIAGNOSTIC PRINT (Item 4)
            print("\n--- Canine Classification Logic ---")
            incisor_indices_in_arch = sorted([t['angular_order'] for t in final_incisors])
            print(f"  Incisor angular orders in arch: {incisor_indices_in_arch}")
            canine_candidates_list = [] # Renamed from can_cand to avoid conflict
            if incisor_indices_in_arch:
                min_idx,max_idx = incisor_indices_in_arch[0],incisor_indices_in_arch[-1]
                if min_idx>0:
                    candidate_left = self.teeth[min_idx-1]
                    canine_candidates_list.append(candidate_left)
                    print(f"    Left canine candidate: Tooth {candidate_left['angular_order']} (current type: {candidate_left['type']}) is a canine candidate.")
                if max_idx<num_teeth-1:
                    candidate_right = self.teeth[max_idx+1]
                    canine_candidates_list.append(candidate_right)
                    print(f"    Right canine candidate: Tooth {candidate_right['angular_order']} (current type: {candidate_right['type']}) is a canine candidate.")
            for t in canine_candidates_list:
                if t['type']=='unknown':
                    t['type']='canine'
                    print(f"    Canine classification: Tooth {t['angular_order']} assigned type: canine (adjacent to incisors)")

            # DIAGNOSTIC PRINT (Item 5)
            print("\n--- Fallback Classification (Premolars/Molars by position ratio) ---")
            for i_t, t_obj in enumerate(self.teeth): # i_t is the current index in self.teeth (which is sorted by angle)
                if t_obj['type']=='unknown':
                    # Use angular_order for position_ratio as self.teeth is sorted by angle
                    pos_ratio = t_obj['angular_order']/(num_teeth-1) if num_teeth>1 else 0.5
                    original_type = t_obj['type'] # Should be 'unknown' here
                    if pos_ratio<0.15 or pos_ratio>0.85: t_obj['type']='second_molar'
                    elif pos_ratio<0.25 or pos_ratio>0.75: t_obj['type']='first_molar'
                    elif pos_ratio<0.35 or pos_ratio>0.65: t_obj['type']='second_premolar'
                    else: t_obj['type']='first_premolar'
                    print(f"    Fallback classification (1st pass): Tooth {t_obj['angular_order']} (pos_ratio {pos_ratio:.2f}) assigned type: {t_obj['type']} (was {original_type})")

            print("\n--- Second Pass Fallback (if any unknowns left by specific position ratio) ---")
            for i_t, t_obj in enumerate(self.teeth):
                 if t_obj['type']=='unknown':
                    original_type = t_obj['type']
                    pos_ratio = t_obj['angular_order']/(num_teeth-1) if num_teeth>1 else 0.5
                    # Simplified fallback for any truly missed ones based on general position
                    if pos_ratio < 0.33: t_obj['type'] = 'first_premolar'
                    elif pos_ratio > 0.66: t_obj['type'] = 'first_molar'
                    else: t_obj['type'] = 'second_premolar'
                    print(f"    Fallback classification (2nd pass): Tooth {t_obj['angular_order']} (pos_ratio {pos_ratio:.2f}) assigned type: {t_obj['type']} (was {original_type})")

        # DIAGNOSTIC PRINT (Item 6 - Final Summary)
        print("\n--- Teeth after final classification (Summary) ---")
        for tooth in self.teeth: # angular_order should be present on all teeth
            print(f"  Tooth angular_order: {tooth.get('angular_order', 'N/A')}, Type: {tooth['type']}, Angle: {np.degrees(tooth['angle']):.1f}°, AP: {tooth['ap_position']:.1f}, Label: {tooth['label']}")
    ...
    def calculate_bracket_positions(self):
        ...
        for i,t in enumerate(self.teeth):
            print(f"  Bracket for Tooth {t.get('angular_order', i)} (Type: {t['type']}, Original Label: {t.get('label','N/A')}, Center: {t['center'].round(2)})")
            ...
            b_pos = b_pos_intermediate.astype(float)
            t2b=b_pos-t_c; t2b[self.height_axis]=0
            ...
            if out_dir[self.height_axis] != 0:
                ...
                if norm_val > 1e-8: out_dir=out_dir/norm_val
                else: out_dir = np.array([0,1,0], dtype=float)
            b_pos+=out_dir*2.5
    ...
    def add_clinical_bends(self, archform):
        ...
        if len(c_wire)>2:
            ...
            print("\n--- Clinical wire after initial angular sort: ---")
            for idx, p_info in enumerate(c_wire): print(f"  Idx {idx}: Type: {p_info['tooth_type']}, Pos: {p_info['position'].round(2)}, ToothIdx (angular_order): {p_info['tooth_index']}")
            max_g,g_idx=0,-1
            for i in range(len(c_wire)):
                d=np.linalg.norm(c_wire[(i+1)%len(c_wire)]['position']-c_wire[i]['position'])
                if d>max_g: max_g=d;g_idx=i
            print(f"Max gap {max_g:.1f}mm at index {g_idx} (before reordering).")
        ...
        if c_wire:
            print(f"Wire starts with: Idx 0, Type: {c_wire[0]['tooth_type']}, Pos: {c_wire[0]['position'].round(2)}, ToothIdx (angular_order): {c_wire[0]['tooth_index']}")
            print(f"Wire ends with: Idx {len(c_wire)-1}, Type: {c_wire[-1]['tooth_type']}, Pos: {c_wire[-1]['position'].round(2)}, ToothIdx (angular_order): {c_wire[-1]['tooth_index']}")
        return c_wire
    ...
    def create_wire_mesh(self, clinical_wire):
        ...
        print(f"Wire mesh using {len(positions)} points. First 5: {positions[:5].round(2)}")
        ...
        for i in range(len(positions)-1):
            ...
            print(f"  Segment from point {i} ({p1.round(2)}) to {i+1} ({p2.round(2)}): Dist: {seg_l:.2f}mm. ",end="")
            ...
            else: R=np.identity(3)
            cyl.rotate(R,center=np.array([0,0,0])); cyl.translate(p1+seg_v/2)
            wire_m+=cyl; segments_created+=1
            if i==0: wire_m+=o3d.geometry.TriangleMesh.create_sphere(self.wire_radius*1.05).translate(p1)
            wire_m+=o3d.geometry.TriangleMesh.create_sphere(self.wire_radius*1.05).translate(p2)
        if len(positions)>0:
            print(f"Adding large terminal sphere at wire start: {positions[0].round(2)}")
            wire_m+=o3d.geometry.TriangleMesh.create_sphere(self.wire_radius*1.3).translate(positions[0])
            if len(positions)>1:
                print(f"Adding large terminal sphere at wire end: {positions[-1].round(2)}")
                wire_m+=o3d.geometry.TriangleMesh.create_sphere(self.wire_radius*1.3).translate(positions[-1])
    ...
    def create_bracket_markers(self):
        ...
        for bracket in self.bracket_positions:
            ...
            marker_box.translate([-0.75, -1.0, -0.5])
            marker_box.translate(bracket['position'])
            ...
            else: color_rgb = [0.2, 0.8, 0.2]  # Green
    ...
    def generate_wire(self):
        ...
        wire_mesh, _ = self.create_wire_mesh(clinical_wire_points)
        ...
        if bracket_marker_data_list:
            for marker_data in bracket_marker_data_list:
                ...
                marker_mesh.paint_uniform_color(marker_data['color'])
    ...
    def visualize(self, components):
        ...
        for key, geom in components.items():
            if geom is not None and isinstance(geom, o3d.geometry.Geometry) and geom.has_vertices():
                 vis.add_geometry(geom)
            elif geom is not None and key == 'mesh' and isinstance(geom, o3d.geometry.TriangleMesh) and geom.has_vertices():
                 vis.add_geometry(geom)
...
def run_tests():
    ...
    generator.teeth = [
        {'type': 'canine',           'center': np.array([15,15,0]), 'vertices': np.array([[15,15,1]]), 'angle': -0.5, 'ap_position': 15, 'label':4, 'angular_order': 0},
        {'type': 'lateral_incisor',  'center': np.array([-7,18,0]), 'vertices': np.array([[-7,18,1]]), 'angle': -0.2, 'ap_position': 18, 'label':3, 'angular_order': 1},
        {'type': 'central_incisor',  'center': np.array([-2,19,0]), 'vertices': np.array([[-2,19,1]]), 'angle': -0.05,'ap_position': 19, 'label':1, 'angular_order': 2},
        {'type': 'central_incisor',  'center': np.array([2,19,0]),  'vertices': np.array([[2,19,1]]),  'angle': 0.05, 'ap_position': 19, 'label':0, 'angular_order': 3},
        {'type': 'lateral_incisor',  'center': np.array([7,18,0]),  'vertices': np.array([[7,18,1]]),  'angle': 0.2,  'ap_position': 18, 'label':2, 'angular_order': 4},
    ...
    dummy_verts_np = np.array([[-40,-20,-5],[40,20,5],[0,0,0],[1,1,1],[5,5,5]])
    dummy_triangles_np = np.array([[0,2,1],[0,3,2],[0,1,4],[1,2,4]])
    ...
    instructions = generator.calculate_bending_instructions(optimized_wire)
    ...
    wire_mesh, segments_created = generator.create_wire_mesh(optimized_wire)
    if len(optimized_wire) >= 2:
        assert segments_created == len(optimized_wire) - 1, \
         ...
         assert segments_created == 0, "Expected 0 segments for 1 point wire"
    ...
    if bracket_marker_data_list:
        ...
        assert canine_marker_data['color'] == [0.8, 0.8, 0.2], f"Canine color incorrect: {canine_marker_data['color']}"
...
def test_color_visualization():
    ...
    for bracket in mock_brackets:
        ...
        if 'incisor' in bracket['tooth_type']:
            ...
            marker_box.paint_uniform_color([0.8, 0.8, 0.2]); color_applied="Yellow ([0.8,0.8,0.2])" # Changed back to Yellow
            if bracket['tooth_type']=='canine' and color_applied=="Yellow ([0.8,0.8,0.2])": canine_color_check_passed=True
        else:
    ...
    if canine_color_check_passed: print("\nCanine color (Yellow) correctly processed in logic for this test.")
    else: print("\nERROR: Canine color (Yellow) NOT correctly processed for this test.")
...
def main():
    ...
    if result:
        ...
        if result.get('mesh') is not None:
            ...
            print("Skipping visualization as main mesh is missing.")
